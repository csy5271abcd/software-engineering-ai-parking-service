...
현재 프론트엔드는 Figma Make reference 이미지를 기준으로 주요 사용자 흐름을 구현 중이다.

#### Home 화면

- View 기반 지도 placeholder 구현
- 현재 위치 표시
- 주차장 marker 표시
...
- NFC START mock 성공 후 `startedAt` 타임스탬프 포함하여 ActiveSessionScreen 이동
- ActiveSessionScreen: 실제 입차 시각 기준 경과 시간 타이머, 예상 요금 실시간 표시
- "NFC로 이용 종료·결제" CTA → NFCScanModal(mode='END') 표시
- NFC END mock 성공 후 `endedAt / durationMinutes / finalAmount` 계산 → PaymentScreen 이동
- PaymentScreen: 실제 입차/출차 시각·이용 시간·최종 금액 표시, 결제 수단 선택
- 결제하기 CTA → PaymentResultScreen: 실제 결제 금액·수단·동적 승인 번호 표시
- 결제 완료 후 ParkingTab(이용 내역) 화면으로 이동
...
현재 프론트엔드 구현 흐름은 `v1.1.12` 기준으로 정리한다.
...
| `v1.1.10` | 추천 탭 — 공급자 화면 전환, AI 혼잡도 대시보드 1차 구현                                                   |
| `v1.1.11` | 추천 탭 AI 대시보드 확장 — 시나리오 chip, BEST 카드, 비교 카드 연동                                       |
