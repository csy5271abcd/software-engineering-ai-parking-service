...
- `docs/test/과제6.인스팩션보고서.md` 추가
- `docs/test/과제6.인스팩션보고서.pdf` 추가
- SmartPark 과제6 인스팩션 보고서 작성 완료
- 과제5 소프트웨어설계서를 주 검토 문서로 선정하고, 과제1~4 선행 문서와 README, CHANGELOG, 프론트엔드/백엔드/AI 변경 이력 문서를 함께 검토
- 인스팩션 개요, 준비 체크리스트, 회의 정보, 결함 요약, 결함 목록, 수정 계획, 인스팩션 결론으로 문서 구성
- 결함 유형별 집계 및 Major/Minor 심각도별 집계 작성
- 총 36건의 결함을 요구사항, 설계, 인터페이스/API, 데이터/AI, 구현 이력/형상관리, 문서 형식/오타 관점으로 분류
- 최종 판정은 `Conditionally accept`로 정리
- 즉시 수정 항목과 후속 구현 단계 반영 항목을 구분하여 수정 계획 작성
- `README.md`의 과제6 상태를 완료로 갱신하고, 인스팩션 보고서 산출물 위치와 작성 결과를 반영
